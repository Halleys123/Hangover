...
<div
	class="min-h-screen flex flex-col bg-white dark:bg-zinc-950 transition-colors duration-200"
>
	...
	<div class="flex-1 flex gap-0 h-[calc(100vh-73px)]">
		...
		{#if leftCollapsed}
			...
			>
				<svg
					class="w-4 h-4 text-gray-400 dark:text-zinc-500 group-hover:text-blue-600 dark:group-hover:text-blue-400"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
				>
		...
		{:else}
			<div
				class="flex flex-col border-r border-gray-200 dark:border-zinc-800 bg-[#FAFAFA] dark:bg-zinc-900 shrink-0"
				style="width: {leftWidth}px;"
			>
				<!-- Chat Header -->
				<div
					class="px-4 py-3 border-b border-gray-200 dark:border-zinc-800 flex items-center gap-2 bg-white dark:bg-zinc-900/90"
				>
					<svg
					...
					{#each messages as message (message)}
						...
						>
							{#if message.role === "assistant"}
								<div
									class="w-6 h-6 rounded bg-blue-100 dark:bg-blue-950/80 text-blue-600 dark:text-blue-400 flex-shrink-0 flex items-center justify-center mt-1 border border-blue-200/50 dark:border-blue-800/50"
								>
									<svg
										class="w-4 h-4"
										fill="currentColor"
							...
							{#if message.role === "user"}
								<div
									class="w-6 h-6 rounded-full bg-gray-300 dark:bg-zinc-700 flex-shrink-0 mt-1 flex items-center justify-center"
								>
									<svg
				...
				<!-- Input Area -->
				<div
					class="p-4 bg-white dark:bg-zinc-900 border-t border-gray-200 dark:border-zinc-800"
				>
		...
		<!-- Right: Projects List -->
		<div
			class="flex-1 bg-gray-50 dark:bg-zinc-950 p-8 overflow-y-auto w-full"
		>
			<div class="max-w-4xl mx-auto">
				<div class="flex justify-between items-center mb-8">
					<div>
						...
						<p
							class="text-slate-500 dark:text-zinc-400 text-sm mt-1"
						>
							Pick up where you left off or start a new hardware
							workspace.
						</p>
					</div>
					<button
				...
				<div class="mb-6 relative">
					<svg
						class="w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-zinc-500"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
				...
				{#if projectsLoading}
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each [1, 2, 3] as i (i)}<div
								class="border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-lg p-5 animate-pulse"
							>
								<div
									class="h-5 bg-gray-200 dark:bg-zinc-800 rounded w-3/4 mb-3"
								></div>
								<div
									class="h-3 bg-gray-100 dark:bg-zinc-800 rounded w-full mb-2"
								></div>
								<div
									class="h-3 bg-gray-100 dark:bg-zinc-800 rounded w-2/3"
								></div>
							</div>{/each}
					</div>
				{:else}
						...
						{#each filteredProjects as project (project._id)}
							...
							>
								...
								>
									...
									<span
										class="px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wide {project.status ===
										'completed'
								...
								<p
									class="text-gray-500 dark:text-zinc-400 text-sm mt-1 mb-4 flex-grow leading-relaxed"
								>
									{project.description}
								</p>
								...
								<div class="flex flex-wrap gap-2 mt-auto">
									{#each project.components as component (component)}
										<span
											class="px-2.5 py-1 bg-gray-50 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 text-xs rounded-md border border-gray-200 dark:border-zinc-700"
										>
											{component}
										</span>
									{/each}
								...
								<div
									class="mt-4 pt-3 border-t border-gray-100 dark:border-zinc-800/80 text-xs text-gray-400 dark:text-zinc-500 flex items-center gap-1"
								>
									<svg
										class="w-3.5 h-3.5"
										fill="none"
					...
					{#if filteredProjects.length === 0 && !projectsLoading}
						<div
							class="text-center py-16 bg-white dark:bg-zinc-900 rounded-lg border border-gray-200 dark:border-zinc-800 mt-4 shadow-sm"
						>
							{#if projects.length === 0}
								<p
									class="text-gray-500 dark:text-zinc-400 mb-2 font-medium"
								>
									No projects yet
								</p>
								<button
							...
							{:else}
								<p
									class="text-gray-500 dark:text-zinc-400 font-medium"
								>
